...
namespace AIGen
{
    class RaiseSources : CompositeGoal
    {
        ...
        private int[] SumSourceList(List<ISourceAll> list)
        {
            ...
            {
                sum = AIGen.Sum2Vectors(sum, s.GetAsArray());
            }
            ...
            do
            {
                ...
                for (int loop1 = 0; loop1 < 5; loop1++)
                {
                       ...
                       dontHaveToChange[loop1])
                    {
                        ...
                        turn++;
                        sourceNow = AIGen.Sum2Vectors(sourceNow, sourcePerTurn);
                    }
                }
        ...
        {
            {
                ...
                {
                    if (AIGen.SumVector(map.GetPlayerMe().GetSource().GetAsArray()) * 3.0 / 2.0 > AIGen.SumVector(map.GetPrice(PriceKind.AStealSources).GetAsArray()))
                    {
                        turn = 0;
                    }
                    else if(turn > 1)
